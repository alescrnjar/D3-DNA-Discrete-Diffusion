...
class DDiTBlock(nn.Module):
    ...
    def forward(self, x, rotary_cos_sin, c, seqlens=None):
        ...
        ######Added new PyTorch flash attention
        q = qkv[:, :, 0, :, :]
        k = qkv[:, :, 1, :, :]
        v = qkv[:, :, 2, :, :]
        # Below lines are required to modify the conventional layout to avoid memory-intensive transpose operation
        #source : https://blog.eleuther.ai/rotary-embeddings/ (actually used in GPT-NeoX (PyTorch))
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            q, k = rotary.apply_rotary_pos_emb(
                q, k, cos.to(qkv.dtype), sin.to(qkv.dtype)
            )
        # print (q.shape, k.shape)
        # qkv = rearrange(qkv, 'b s ... -> (b s) ...')
        # print (qkv.shape) #torch.Size([1992, 3, 12, 64])
...
class EmbeddingLayer(nn.Module):
    def __init__(self, dim, vocab_dim):
        ...
        #self.signal_embedding = nn.Linear(3, dim)  # remove if label embedding is used
        self.signal_embedding = nn.Linear(1, dim)  # remove if label embedding is used #AC : 1 (or 3) is number of outputs
        torch.nn.init.kaiming_uniform_(self.embedding, a=math.sqrt(5))

    #def forward(self, x): #VANILLA/C
    def forward(self, x, y): #V/CONDITIONING
        # print (x.shape, x.dtype)
        # print (y.shape, y.dtype)
        vocab_embed = self.embedding[x]  # return only this if label embedding is used
        signal_embed = self.signal_embedding(y.to(torch.float32))  #V/CONDITIONING
        # print (vocab_embed.shape)
        # print (signal_embed.shape)
        return torch.add(vocab_embed, signal_embed[:, None, :])  # [:, None, :] extra for deepstarr  #V/CONDITIONING
        #return vocab_embed  # [:, None, :] extra for deepstarr  #VANILLA/C
...
class SEDD(nn.Module, PyTorchModelHubMixin):
    def __init__(self, config):
        ...
        self.blocks = nn.ModuleList([
            DDiTBlock(config.model.hidden_size, config.model.n_heads, config.model.cond_dim,
        ...
        self.label_emb = nn.Sequential(
            nn.Linear(2, config.model.length), #change to 2 for transformer model inference
            nn.SiLU(),
    ...
    #def forward(self, indices, train, sigma): #VANILLA/C
    def forward(self, indices, labels, train, sigma):  #V/CONDITIONING
        # Uncomment below for original transformer based networks
        #x = self.vocab_embed(indices) #VANILLA/C
        x = self.vocab_embed(indices, labels) #V/CONDITIONING
